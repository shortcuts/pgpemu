...
int main() {
    ...
    test_assert(!nvs_read_check("TEST", ESP_FAIL, "read_error"), "nvs_read_check returns false on generic ESP_FAIL");
    ...
    test_assert(!nvs_read_check("TEST", -999, "unknown_error"), "nvs_read_check returns false on unknown error code");
    ...
    test_assert(!nvs_write_check("TEST", ESP_FAIL, "write_failed"), "nvs_write_check returns false on ESP_FAIL");
    ...
    test_assert(!nvs_read_check("TEST", ESP_ERR_NVS_NOT_FOUND, "key") && nvs_write_check("TEST", ESP_OK, "key"),
