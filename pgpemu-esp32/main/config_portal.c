...
static esp_err_t root_get(httpd_req_t *r)
{
    ...
    int n = snprintf(page, sizeof(page),
        ...
        "<style>"
        "body { background:#000; color:#fff; font-family:sans-serif; line-height:1.2; margin:10px; }"
        "form { margin:0; }"
        "h2 { margin:0 0 8px; }"
        "input[type=number], button { width:100%%; padding:6px 8px; margin:2px 0; box-sizing:border-box; background:#222; color:#fff; border:1px solid #555; }"
...
{
    if      (!strcmp(key,"d")) set_setting_uint8(&settings.chosen_device, clamp_int(atoi(val),0,9));
    else if (!strcmp(key,"c")) set_setting_uint8(&settings.target_active_connections, clamp_int(atoi(val),1,4));

    else if (!strcmp(key,"ac")) { set_setting_bool(&settings.autocatch, true); seen_ac = true; }
    else if (!strcmp(key,"as")) { set_setting_bool(&settings.autospin, true); seen_as = true; }
    else if (!strcmp(key,"pp")) { set_setting_bool(&settings.powerbank_ping, true); seen_pp = true; }
    else if (!strcmp(key,"ub")) { set_setting_bool(&settings.use_button, true); seen_ub = true; }
    else if (!strcmp(key,"ul")) { set_setting_bool(&settings.use_led, true); seen_ul = true; }
    else if (!strcmp(key,"sl")) { set_setting_bool(&settings.led_interactions, true); seen_sl = true; }
}
...
static esp_err_t save_post(httpd_req_t *r)
{
    ...
    if (!seen_ac) set_setting_bool(&settings.autocatch, false);
    if (!seen_as) set_setting_bool(&settings.autospin, false);
    if (!seen_pp) set_setting_bool(&settings.powerbank_ping, false);
    if (!seen_ub) set_setting_bool(&settings.use_button, false);
    if (!seen_ul) set_setting_bool(&settings.use_led, false);
    if (!seen_sl) set_setting_bool(&settings.led_interactions, false);
...
static void wifi_ap_init(void)
{
    ...
    wifi_config_t ap = {.ap = {
        .ssid = PGPEMU_SSID, .authmode = WIFI_AUTH_OPEN,
        .ssid_len = sizeof(PGPEMU_SSID) - 1, .channel = 1, .max_connection = 4 }};
