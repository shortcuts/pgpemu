...
void init_led_output() {
    ...
    io_conf.pin_bit_mask = (1 << CONFIG_GPIO_OUTPUT_RED) | (1 << CONFIG_GPIO_OUTPUT_GREEN) |
                           (1 << CONFIG_GPIO_OUTPUT_BLUE);
...
static void led_output_task(void* pvParameters) {
    ...
    while (true) {
        if (xQueueReceive(led_queue, &led_event, portMAX_DELAY)) {
            ESP_LOGD(LEDOUTPUT_TAG, "setting rgb=(%d, %d, %d)", led_event.red, led_event.green,
                     led_event.blue);
