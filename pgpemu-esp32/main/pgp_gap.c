...
void advertise_if_needed() {
    ...
    if (get_active_connections() < target_active_connections) {
        ...
        ESP_LOGI(BT_GAP_TAG, "not advertising again, %d connections reached",
                 target_active_connections);
    }
}

void pgp_advertise() { esp_ble_gap_start_advertising(&adv_params); }

void pgp_advertise_stop() { esp_ble_gap_stop_advertising(); }
